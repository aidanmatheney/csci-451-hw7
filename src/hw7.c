...
struct ProcessTransactionsThreadStartArg {
    struct HW7TransactionRecord const *transactionRecordPtr;
    float *balancePtr;
...
/**
 * Run CSCI 451 HW7. This uses the given transaction records to model multithreaded deposit and withdrawal transactions
 ...
 */
void hw7(struct HW7TransactionRecord const * const transactionRecords, size_t const transactionRecordCount) {
    ensureInitialized();

    guardNotNull(transactionRecords, "transactionRecords", "hw7");

    float balance = 0;

    pthread_mutex_t balanceMutex;
    safeMutexInit(&balanceMutex, NULL, "hw7");

    struct ProcessTransactionsThreadStartArg * const threadStartArgs = (
        safeMalloc(sizeof *threadStartArgs * transactionRecordCount, "hw7")
    );
    pthread_t * const threadIds = safeMalloc(sizeof *threadIds * transactionRecordCount, "hw7");
    for (size_t i = 0; i < transactionRecordCount; i += 1) {
        struct HW7TransactionRecord const * const transactionRecordPtr = &transactionRecords[i];
    ...
    for (size_t i = 0; i < transactionRecordCount; i += 1) {
        ...
        safePthreadJoin(threadId, "hw7");
    }

    free(threadStartArgs);
    free(threadIds);

    safeMutexDestroy(&balanceMutex, "hw7");
...
static void ensureInitialized(void) {
    ...
    safeRegcomp(&beginTransactionSectionRegex, "^R$", REG_EXTENDED | REG_NOSUB, "hw7 ensureInitialized");
    // ^[+-][0-9]+\.?[0-9]*|[0-9]*\.[0-9]+$
    safeRegcomp(&transactionRegex, "^[+-][0-9]+\\.?[0-9]*|[0-9]*\\.[0-9]+$", REG_EXTENDED | REG_NOSUB, "hw7 ensureInitialized");
    safeRegcomp(&endTransactionSectionRegex, "^W$", REG_EXTENDED | REG_NOSUB, "hw7 ensureInitialized");
...
static void *processTransactionsThreadStart(void * const argAsVoidPtr) {
    ...
    FILE * const transactionFile = (
        safeFopen(argPtr->transactionRecordPtr->filePath, "r", "hw7 processTransactionsThreadStart")
    );
    ...
    while (true) {
        ...
        if (regexec(&beginTransactionSectionRegex, beginTransactionSectionLine, 0, NULL, 0) == REG_NOMATCH) {
            abortWithErrorFmt(
                "hw7 processTransactionsThreadStart: %s thread failed to parse BeginTransactionSection symbol from \"%s\" (line: \"%s\")",
        ...
        safeMutexLock(argPtr->balanceMutexPtr, "hw7 processTransactionsThreadStart");
        ...
        while (true) {
            ...
            if (line == NULL) {
                abortWithErrorFmt(
                    "hw7 processTransactionsThreadStart: %s thread reached EOF before EndTransactionSection symbol was parsed from \"%s\"",
                    argPtr->transactionRecordPtr->name,
                    argPtr->transactionRecordPtr->filePath
                );
                break;
            ...
            if (regexec(&transactionRegex, line, 0, NULL, 0) == REG_NOMATCH) {
                abortWithErrorFmt(
                    "hw7 processTransactionsThreadStart: %s thread failed to parse Deposit, Withdraw, or EndTransactionSection symbol from \"%s\" (line: \"%s\")",
                    argPtr->transactionRecordPtr->name,
                    argPtr->transactionRecordPtr->filePath,
                    line
                );
        ...
        *argPtr->balancePtr = balance;
        ...
        safeMutexUnlock(argPtr->balanceMutexPtr, "hw7 processTransactionsThreadStart");
